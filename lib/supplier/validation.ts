import { z } from "zod";
import { ALLOWED_DOCUMENT_MIME_TYPES, BUSINESS_TYPES, BRANCH_TYPES, DELIVERY_HANDLERS, DOCUMENT_TYPES, MAX_DOCUMENT_SIZE_BYTES, SETTLEMENT_METHODS } from "@/lib/supplier/constants";
...
export const businessInformationSchema = z.object({
  ...
  businessDescription: z.string().trim().min(10, "Add a short description of the business"),
  yearEstablished: z.coerce.number().int().min(1900, "Enter a valid year").max(new Date().getFullYear(), "Year cannot be in the future"),
  website: z.string().trim().optional().default(""),
  primaryCategories: z.array(z.string()).min(1, "Select at least one product category"),
...
export const contactInformationSchema = z.object({
  primaryContactName: z.string().trim().min(2, "Enter the primary contact name"),
...
export const registrationComplianceSchema = z.object({
  registrationNumber: z.string().trim().min(2, "Enter the business registration number"),
  ...
  if (data.vatRegistered && !data.vatNumber) {
    ctx.addIssue({ code: "custom", path: ["vatNumber"], message: "Enter the VAT registration number" });
  }
});
export type RegistrationComplianceInput = z.infer<typeof registrationComplianceSchema>;
...
export const deliveryCoverageSchema = z.object({
  ...
  standardLeadTimeDays: z.coerce.number().int().min(0, "Enter the standard lead time"),
...
export const settlementSchema = z.object({
  ...
  if (data.settlementMethod === "bank") {
    if (!data.bankName) ctx.addIssue({ code: "custom", path: ["bankName"], message: "Enter the bank name" });
    if (!data.accountName) ctx.addIssue({ code: "custom", path: ["accountName"], message: "Enter the account name" });
    ...
    if (!data.momoNetwork) ctx.addIssue({ code: "custom", path: ["momoNetwork"], message: "Select a mobile money network" });
    ...
    if (!data.momoAccountName) ctx.addIssue({ code: "custom", path: ["momoAccountName"], message: "Enter the registered mobile money name" });
  }
});
...
export const documentUploadSchema = z.object({
  ...
  mimeType: z.enum(ALLOWED_DOCUMENT_MIME_TYPES, { message: "Only PDF, JPG and PNG files are allowed" }),
  fileSize: z.number().int().positive().max(MAX_DOCUMENT_SIZE_BYTES, "File must be 10MB or smaller")
});
