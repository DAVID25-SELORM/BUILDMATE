export type CalculatorKind="blocks"|"concrete"|"paint"|"tiles"|"roofing"|"plaster";
export type EstimateInput={length:number;width:number;height:number;openings:number;waste:number;unitPrice:number};
export type Estimate={quantity:number;unit:string;label:string;cost:number;details:string[]};
