...
export async function changeOrganisationAssignments(scope:OrganisationScope,membershipId:string,formData:FormData){const permission=scope==="supplier"?"supplier.staff.manage":"staff.manage";const{supabase}=await context(scope,permission);const{error}=await supabase.rpc("set_organisation_member_assignments",{target_membership:membershipId,target_branch_ids:formData.getAll("branchIds").map(String),target_warehouse_ids:formData.getAll("warehouseIds").map(String),target_project_ids:formData.getAll("projectIds").map(String),target_approval_limit:formData.get("approvalLimit")?Number(formData.get("approvalLimit")):null,target_reason:String(formData.get("reason")??"")});if(error)throw new Error(error.message);revalidatePath(`${basePath(scope)}/${membershipId}`);}
export async function changeOrganisationPermission(scope:OrganisationScope,membershipId:string,permissionKey:string,formData:FormData){const permission=scope==="supplier"?"supplier.staff.manage":"staff.manage";const{supabase}=await context(scope,permission);const{error}=await supabase.rpc("set_organisation_permission_override",{target_membership:membershipId,target_permission_key:permissionKey,should_grant:formData.get("granted")==="true",target_reason:String(formData.get("reason")??"")});if(error)throw new Error(error.message);revalidatePath(`${basePath(scope)}/${membershipId}`);}
export async function transferOrganisationOwnership(scope:OrganisationScope,fromMembership:string,formData:FormData){const permission=scope==="supplier"?"supplier.staff.manage":"staff.manage";const{supabase}=await context(scope,permission);const{error}=await supabase.rpc("transfer_organisation_ownership",{from_membership:fromMembership,to_membership:String(formData.get("toMembership")??""),target_reason:String(formData.get("reason")??"")});if(error)throw new Error(error.message);revalidatePath(basePath(scope));redirect(basePath(scope));}
export async function revokeOrganisationInvitation(scope:OrganisationScope,invitationId:string,formData:FormData){const permission=scope==="supplier"?"supplier.staff.manage":"staff.manage";const{supabase}=await context(scope,permission);const{error}=await supabase.rpc("revoke_invitation",{target_invitation:invitationId,target_reason:String(formData.get("reason")??"")});if(error)throw new Error(error.message);revalidatePath(basePath(scope));}
export async function resendOrganisationInvitation(scope:OrganisationScope,invitationId:string,formData:FormData){const permission=scope==="supplier"?"supplier.staff.invite":"staff.invite";const{supabase,organisationId}=await context(scope,permission);const{data}=await supabase.rpc("list_organisation_invitations",{target_organisation:organisationId});const invitation=(data??[]).find((row:{invitation_id:string})=>row.invitation_id===invitationId)as{email:string;full_name:string;role_key:string}|undefined;if(!invitation)throw new Error("Invitation not found");const{token,tokenHash}=generateInvitationToken();const{error}=await supabase.rpc("resend_invitation",{target_invitation:invitationId,target_token_hash:tokenHash,target_reason:String(formData.get("reason")??"")});if(error)throw new Error(error.message);await sendNotification({channel:"email",template_key:"staff_invitation",payload:{fullName:invitation.full_name,roleLabel:invitation.role_key.replaceAll("_"," "),inviteUrl:invitationUrl(token)},recipient_email:invitation.email,recipient_phone:null});revalidatePath(basePath(scope));}
