...
export default async function ShopPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; location?: string; sort?: string }> }) {
  ...
  for (const listing of (rawListings ?? []) as unknown as Listing[]) {
    ...
    if (existing) { existing.supplierIds.add(listing.supplier_id); existing.lowestPrice = Math.min(existing.lowestPrice, Number(listing.price)); if (!existing.imageUrl && mediaUrl) { existing.imageUrl = mediaUrl; existing.imageAlt = media?.alt_text; } }
    else grouped.set(listing.product_id, { productId: listing.product_id, name: listing.products.name, category: listing.products.categories?.name ?? "Materials", unit: listing.products.base_unit, lowestPrice: Number(listing.price), supplierIds: new Set([listing.supplier_id]), imageUrl: mediaUrl, imageAlt: media?.alt_text });
  }
  const cards = [...grouped.values()].map(product => ({...product,supplierCount:product.supplierIds.size})).sort((a, b) => sort === "price" ? a.lowestPrice - b.lowestPrice : b.supplierCount - a.supplierCount || a.lowestPrice - b.lowestPrice);
