...
type Offer = {
  ...
  show_exact_stock_to_customers: boolean;
...
export default async function ProductOffersPage({
  ...
  const [{ data: product }, { data: rawOffers }] = await Promise.all([
    supabase
      .from("products")
      .select(
        "id,name,description,base_unit,specifications,images,categories(name),brands(name)",
      )
      .eq("id", productId)
      .eq("is_active", true)
      .maybeSingle(),
  ...
  const offers = ((rawOffers ?? []) as unknown as Offer[]).filter(
    (offer) =>
      (!offer.price_effective_date || offer.price_effective_date <= today) &&
    ...
    <section className="container-shell py-12">
      ...
      <div className="mt-12">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black">Compare supplier offers</h2>
        ...
        <div className="mt-6 space-y-4">
          ...
          {!offers.length && (
            <div className="card p-10 text-center text-slate-500">
              ...
              Request a quote and BuildMate will help source it.
            </div>
          )}
        </div>
      </div>
