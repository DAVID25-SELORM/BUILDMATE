...
type Offer = {
  ...
  stock_status: string; lead_time_days: number; minimum_order_quantity: number | string | null;
    ...
    <div className="mt-12">
      ...
      <div className="mt-6 space-y-4">
        {offers.map((offer) => <article className="card grid gap-5 p-5 lg:grid-cols-[1fr_auto_auto] lg:items-center" key={offer.id}>
          <div><div className="flex items-center gap-2"><h3 className="text-lg font-bold">{offer.organisations.name}</h3><BadgeCheck className="h-5 w-5 text-brand-600" aria-label="Approved supplier" /></div><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-slate-600"><span className="flex items-center gap-1"><PackageCheck className="h-4 w-4" />{offer.stock_quantity == null && offer.stock_status === "confirmation_required" ? "Confirm availability with supplier" : offer.stock_quantity == null ? offer.stock_status.replaceAll("_", " ") : `${offer.stock_quantity} available`}</span>{offer.supplier_branches && <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />{offer.supplier_branches.name}, {offer.supplier_branches.city}</span>}<span className="flex items-center gap-1"><Clock3 className="h-4 w-4" />{offer.lead_time_days === 0 ? "Same day" : `${offer.lead_time_days} day lead time`}</span>{offer.delivery_available && <span className="flex items-center gap-1"><Truck className="h-4 w-4" />Delivery</span>}{offer.pickup_available && <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />Pickup</span>}</div>{offer.supplier_notes && <p className="mt-3 text-sm text-slate-600">{offer.supplier_notes}</p>}</div>
