...
export default async function SupplierPortalPreview({
  params,
}: {
  ...
  const permissionBySection: Record<string, string> = {
    orders: "orders.view",
    quotes: "quotations.view",
    products: "products.view",
    settlements: "settlements.view",
  };
  const permissionChecks = await Promise.all(
    Object.entries(permissionBySection).map(
  ...
  const visible = Object.fromEntries(permissionChecks) as Record<
    string,
    boolean
  >;
  ...
  const [
    ...
    supabase
      .from("organisations")
      .select("name,verification_status,decision_reason,suspended_reason")
      .eq("id", id)
    ...
    supabase
      .from("supplier_listings")
      .select(
        "id,sku,price,stock_quantity,stock_status,lead_time_days,is_active,products(name,base_unit)",
      )
      .eq("supplier_id", id)
      .order("created_at", { ascending: false }),
    supabase
      .from("quote_requests")
      .select(
        "id,title,delivery_location,required_date,notes,created_at,quote_request_items(description,quantity,unit)",
      )
      .eq("status", "open")
      .order("created_at", { ascending: false }),
    supabase
      .from("supplier_ledger_entries")
      .select("id,entry_type,amount,status,description,created_at,settled_at")
      .eq("supplier_id", id)
      .order("created_at", { ascending: false }),
  ]);
  ...
  if (section === "orders")
    content = <SupplierOrdersView orders={orders ?? []} readOnly />;
  else if (section === "quotes")
    content = <SupplierQuotesView requests={requests ?? []} readOnly />;
  else if (section === "products")
    content = <SupplierProductsView listings={listings ?? []} readOnly />;
  else if (section === "settlements")
    content = (
      <SupplierSettlementsView
        entries={entries}
        balance={settlementBalance(entries)}
      />
    );
  else if (decision.action === "show_dashboard")
    content = (
      <SupplierOverview
        orders={orders ?? []}
      ...
      <>
        ...
        <div className="mt-6">
          <StatusBanner
            status={status}
            reason={
              organisation.decision_reason ?? organisation.suspended_reason
            }
            showAction={false}
          />
        </div>
  ...
  const nav =
    decision.action === "show_dashboard"
      ? ([
          { label: "Overview" },
          visible.orders && { label: "Orders", section: "orders" },
          visible.quotes && { label: "Quotation requests", section: "quotes" },
          visible.products && { label: "Products", section: "products" },
          visible.settlements && {
            label: "Settlements",
            section: "settlements",
          },
      ...
      : [
          { label: "Overview" },
          { label: "Application status", section: "application-status" },
        ];
  return (
    <PortalPreviewShell
      portalType="supplier"
