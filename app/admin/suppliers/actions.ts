...
export async function startSupplierPreview(organisationId:string,formData:FormData){const reason=String(formData.get("reason")??"").trim();if(reason.length<5)throw new Error("A preview reason is required");const h=await headers();const{data,error}=await(await createClient()).rpc("start_admin_portal_preview",{target_portal:"supplier",target_user:null,target_organisation:organisationId,target_reason:reason,target_reference:String(formData.get("referenceNumber")??"")||null,request_ip:h.get("x-forwarded-for")?.split(",")[0]?.trim()??null,request_user_agent:h.get("user-agent")??null});if(error||!data)throw new Error(error?.message??"Unable to start preview");(await cookies()).set("admin_portal_preview",String(data),{httpOnly:true,sameSite:"lax",secure:process.env.NODE_ENV==="production",path:"/",maxAge:1800});redirect(`/admin/preview/supplier/${organisationId}`)}
