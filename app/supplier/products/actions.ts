...
export async function saveListing(formData: FormData) {
  const { user,supabase,membership } = await requireSupplierPermission(formData.get("id") ? "products.edit" : "products.create");
  ...
  const payload = {
    ...
    is_active: parsed.data.listingStatus === "published", availability_confirmed_at: new Date().toISOString(), updated_at: new Date().toISOString(), ...priceMetadata
  };
  if (parsed.data.branchId) {
    const { data: branch } = await supabase.from("supplier_branches").select("id").eq("id", parsed.data.branchId).eq("organisation_id", membership.organisationId).maybeSingle();
    if (!branch) return { error: "Choose a branch belonging to this supplier" };
  }
  if (parsed.data.warehouseId) {
    const { data: warehouse } = await supabase.from("supplier_warehouses").select("id,branch_id").eq("id", parsed.data.warehouseId).eq("organisation_id", membership.organisationId).maybeSingle();
    if (!warehouse) return { error: "Choose a warehouse belonging to this supplier" };
    if (parsed.data.branchId && warehouse.branch_id && warehouse.branch_id !== parsed.data.branchId) return { error: "The warehouse does not belong to the selected branch" };
  }
  const query = parsed.data.id ? supabase.from("supplier_listings").update(payload).eq("id", parsed.data.id).eq("supplier_id", membership.organisationId) : supabase.from("supplier_listings").insert(payload);
...
export async function setListingActive(listingId: string, isActive: boolean) {
  ...
  if (isActive) {
    const [{ data: listing }, { data: organisation }] = await Promise.all([
      supabase.from("supplier_listings").select("price,stock_status,delivery_available,pickup_available").eq("id",listingId).eq("supplier_id",membership.organisationId).maybeSingle(),
    ...
    if (!listing || listing.price == null) throw new Error("Add a retail price before publishing");
    if (listing.stock_status === "out_of_stock") throw new Error("An out-of-stock listing cannot be published");
    if (!listing.delivery_available && !listing.pickup_available) throw new Error("Choose delivery or pickup before publishing");
    if (!organisation || organisation.account_status !== "active" || organisation.verification_status !== "approved" || !organisation.product_publishing_enabled) throw new Error("Supplier publishing is not currently enabled");
  }
...
export async function quickUpdateListing(listingId: string, formData: FormData) {
  const { user, supabase, membership } = await requireSupplierPermission("products.edit");
  ...
  const stockStatus = String(formData.get("stockStatus") ?? "confirmation_required");
...
export async function bulkUpdateListings(formData: FormData) {
  ...
  const listingIds = [...new Set(formData.getAll("listingIds").map(String))].filter((id) => /^[0-9a-f-]{36}$/i.test(id));
  if (!listingIds.length) return { error: "Select at least one product" };
  if (listingIds.length > 100) return { error: "Select no more than 100 products" };
  ...
  if (!['in_stock','low_stock','out_of_stock','confirmation_required','available_on_order'].includes(stockStatus)) return { error: "Choose a valid stock status" };
  if (!['draft','published','out_of_stock','seasonal','discontinued'].includes(listingStatus)) return { error: "Choose a valid listing status" };
  if (branchId) {
    const { data } = await supabase.from("supplier_branches").select("id").eq("id",branchId).eq("organisation_id",membership.organisationId).maybeSingle();
    if (!data) return { error: "Choose a branch belonging to this supplier" };
  }
  if (warehouseId) {
    const { data } = await supabase.from("supplier_warehouses").select("id,branch_id").eq("id",warehouseId).eq("organisation_id",membership.organisationId).maybeSingle();
  ...
  if (listingStatus === "published") {
    ...
    if (publishContext.membership.organisationId !== membership.organisationId) return { error: "Publishing permission is required" };
    if ((owned ?? []).some((item) => item.price == null)) return { error: "Every selected listing needs a price before publishing" };
    if (stockStatus === "out_of_stock") return { error: "Out-of-stock listings cannot be published" };
    if (!deliveryAvailable && !pickupAvailable) return { error: "Choose delivery or pickup before publishing" };
  }
  const { error } = await supabase.from("supplier_listings").update({ branch_id: branchId, warehouse_id: warehouseId, stock_status: stockStatus, listing_status: listingStatus, is_active: listingStatus === "published", delivery_available: deliveryAvailable, pickup_available: pickupAvailable, availability_confirmed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("supplier_id",membership.organisationId).in("id",listingIds);
  if (error) return { error: error.message };
  revalidatePath("/supplier/products"); revalidatePath("/shop");
