...
export default async function SupplierProductsPage() {
  ...
  const [{ data: products }, { data: listings }, { data: branches }, { data: warehouses }, { data: organisation }, { data: clarifications }, { data: canEdit }, { data: canCreate }, { data: canPublish }] = await Promise.all([
    ...
    supabase.from("supplier_price_clarifications").select("id,raw_description,raw_price_text,notes,review_status,confirmed_product_name,confirmed_specification,confirmed_sales_unit,confirmed_price,supplier_action,supplier_note,mapped_product_id,mapped_variant_id").eq("supplier_id",membership.organisationId).eq("status","requires_confirmation").order("created_at"),
    supabase.rpc("has_permission",{target_permission:"products.edit",target_organisation:membership.organisationId}),
    supabase.rpc("has_permission",{target_permission:"products.create",target_organisation:membership.organisationId}),
    supabase.rpc("has_permission",{target_permission:"products.publish",target_organisation:membership.organisationId}),
  ]);
  ...
  const supplierCanPublish = canPublish === true && organisation?.account_status === "active" && organisation?.verification_status === "approved" && organisation?.product_publishing_enabled === true;
  return <DashboardShell title="Supplier portal" nav={await supplierNavigation(membership.organisationId)}>
    ...
    <div className="mt-6"><SupplierInventoryEditor listings={inventoryListings} branches={branches ?? []} warehouses={warehouses ?? []} canEdit={canEdit === true} supplierCanPublish={supplierCanPublish} /></div>
    {!!clarifications?.length && <ClarificationQueue items={clarifications} products={catalogue} />}
    {canEdit === true && <details className="card mt-6 p-5"><summary className="cursor-pointer text-lg font-bold text-brand-800">Advanced listing details</summary><p className="mt-2 text-sm text-slate-600">Edit SKU, wholesale terms, minimum order, lead time, notes and individual fulfilment settings.</p><div className="mt-5 space-y-4">{inventoryListings.map((listing) => <ListingForm key={listing.id} products={catalogue} branches={branches ?? []} warehouses={warehouses ?? []} initial={{ id:listing.id, productId:listing.product_id, sku:listing.sku, price:listing.price, wholesalePrice:listing.wholesale_price, wholesaleMinimum:listing.wholesale_minimum, stockQuantity:listing.stock_quantity, stockStatus:listing.stock_status, inventoryMode:listing.inventory_mode, leadTimeDays:listing.lead_time_days, minimumOrderQuantity:listing.minimum_order_quantity, deliveryAvailable:listing.delivery_available, pickupAvailable:listing.pickup_available, supplierNotes:listing.supplier_notes, listingStatus:listing.listing_status, branchId:listing.branch_id, warehouseId:listing.warehouse_id }} />)}</div></details>}
    {canCreate === true && <details className="mt-6"><summary className="cursor-pointer font-bold text-brand-800">Add more catalogue products</summary><div className="mt-3"><CataloguePicker products={catalogue} /></div></details>}
