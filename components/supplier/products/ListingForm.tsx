...
export function ListingForm({
  ...
  return (
    ...
    >
      <h2 className="text-xl font-bold md:col-span-3">
        {editing ? "Complete draft listing" : "Add one product manually"}
      </h2>
      {initial && <input type="hidden" name="id" value={initial.id} />}
      <label className="md:col-span-2">
        <span className="label">Catalogue product</span>
        <select
          className="input"
          name="productId"
          required
      ...
      <label>
        ...
        <input className="input" name="sku" defaultValue={initial?.sku ?? ""} />
      </label>
      <label>
        <span className="label">Retail price (GHS)</span>
        <input
          className="input"
          name="price"
          type="number"
      ...
      <label>
        <span className="label">Wholesale minimum</span>
        <input
          className="input"
          ...
          min="0"
          step="0.01"
          defaultValue={initial?.wholesaleMinimum ?? ""}
        />
      </label>
      {initial?.inventoryMode === "exact_quantity" ? (
        <div className="rounded-xl bg-brand-50 p-4 text-sm text-brand-900 md:col-span-2">
          Exact stock is movement-controlled. Use <b>Inventory</b> to receive,
          adjust, return or transfer stock.
        </div>
      ) : (
        <label>
          ...
          <select
            className="input"
            name="stockStatus"
            defaultValue={initial?.stockStatus ?? "confirmation_required"}
          >
            <option value="in_stock">In stock</option>
            <option value="low_stock">Low stock</option>
            <option value="out_of_stock">Out of stock</option>
            <option value="confirmation_required">Confirm availability</option>
      ...
      <label>
        ...
        <input
          ...
          type="number"
          min="0"
          defaultValue={initial?.leadTimeDays ?? 1}
          required
        />
      </label>
      <LocationFields
        ...
        initialBranchId={initial?.branchId}
        initialWarehouseId={initial?.warehouseId}
      />
      <fieldset className="flex items-center gap-5 rounded-xl border border-slate-200 px-4 py-3">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            defaultChecked={initial?.pickupAvailable ?? true}
          />{" "}
          Pickup
        </label>
      </fieldset>
      <label>
        ...
        <select
          className="input"
          name="listingStatus"
          defaultValue={initial?.listingStatus ?? "draft"}
        >
      ...
      <label className="md:col-span-3">
        ...
        <textarea
          ...
          name="supplierNotes"
          defaultValue={initial?.supplierNotes ?? ""}
          placeholder="Sizes, packaging, collection instructions or availability notes"
        />
      </label>
      {message && (
        <p className="text-sm font-medium md:col-span-3" role="status">
          {message}
        </p>
      )}
      <button className="btn-primary md:col-span-3" disabled={pending}>
        {pending ? "Saving..." : editing ? "Save listing" : "Create listing"}
      </button>
    </form>
  );
}
