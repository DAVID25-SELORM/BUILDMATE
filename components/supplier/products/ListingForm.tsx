...
export function ListingForm({ products, initial }: { products: ProductOption[]; initial?: ListingInitial }) {
  ...
  return <form className="card grid gap-4 p-6 md:grid-cols-3" action={async (formData) => {
    ...
    <label><span className="label">Retail price (GHS)</span><input className="input" name="price" type="number" min="0" step="0.01" defaultValue={initial?.price ?? ""} required /></label>
