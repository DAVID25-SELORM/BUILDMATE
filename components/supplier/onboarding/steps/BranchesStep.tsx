...
import {
  completeBranchesStep,
  deleteBranch,
  saveBranch,
} from "@/app/supplier/onboarding/actions";
...
export function BranchesStep({
  ...
  async function handleFormSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (
      parsed.data.isMainBranch &&
      wouldCreateDuplicateMainBranch(
        branches.map((b) => ({ id: b.id, isMainBranch: b.is_main_branch })),
        parsed.data.id ?? null,
      )
    ) {
      setFormError(
        "Another branch is already set as the main branch. Unset it first.",
      );
      return;
    }
    ...
    if (parsed.data.id) {
      setBranches((prev) =>
        prev.map((b) =>
          b.id === parsed.data.id
            ? { ...b, ...rowFromInput(parsed.data, b.id!) }
            : b,
        ),
      );
    } else {
      setBranches((prev) => [
        ...prev,
        rowFromInput(parsed.data, crypto.randomUUID()),
      ]);
    }
    setEditing(null);
  }
    ...
    <div className="card space-y-5 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Add every location customers can be served from. Only one can be the
          main branch.
        </p>
      </div>
          ...
          >
            <div>
              ...
              <p className="text-sm text-slate-600">
                {BRANCH_TYPE_LABELS[branch.branch_type]} • {branch.city},{" "}
                {branch.region}
              </p>
            </div>
            <div className="flex gap-2">
              <button
                ...
                onClick={() => setEditing(toBranchInput(branch))}
              >
                Edit
              </button>
              <button
      ...
      {editing ? (
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <Field
              label="Branch or warehouse name"
              htmlFor="branchName"
            ...
            >
              <select
                ...
                onChange={(e) =>
                  setEditing({
                    ...editing,
                    branchType: e.target.value as typeof editing.branchType,
                  })
                }
              >
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field
              label="GhanaPost GPS"
              htmlFor="branchGps"
              error={fieldErrors.ghanaPostGps}
            >
              <input
                id="branchGps"
                className="input"
                value={editing.ghanaPostGps}
                onChange={(e) =>
                  setEditing({ ...editing, ghanaPostGps: e.target.value })
                }
              />
            </Field>
          </div>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setEditing({ ...editing, operatingHours: e.target.value })
              }
            />
          </Field>
          <div className="flex flex-wrap gap-6 text-sm">
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={editing.isMainBranch}
                onChange={(e) =>
                  setEditing({ ...editing, isMainBranch: e.target.checked })
                }
              />{" "}
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={editing.supportsPickup}
                onChange={(e) =>
                  setEditing({ ...editing, supportsPickup: e.target.checked })
                }
              />{" "}
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={editing.supportsDelivery}
                onChange={(e) =>
                  setEditing({ ...editing, supportsDelivery: e.target.checked })
                }
              />{" "}
              Delivers from this branch
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 pt-5">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={continueLoading}
          >
            {continueLoading ? "Saving..." : "Save & continue"}
          </button>
        </div>
      </div>
    </div>
