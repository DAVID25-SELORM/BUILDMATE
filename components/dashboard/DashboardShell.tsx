...
export function DashboardShell({ title, nav, children }: { title: string; nav: NavItem[]; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-100"><header className="border-b bg-white"><div className="container-shell flex flex-col gap-3 py-4 xl:flex-row xl:items-center xl:justify-between"><div className="flex items-center justify-between gap-4"><Link href="/" className="font-black text-brand-700">BuildMate</Link><div className="text-sm font-semibold xl:hidden">{title}</div></div><div className="flex flex-wrap items-center gap-2">{showPortalSwitcher&&<nav aria-label="Preview a user portal" className="flex flex-wrap items-center gap-2"><Link href="/admin" prefetch className="rounded-lg border border-brand-700 bg-brand-700 px-3 py-2 text-xs font-semibold text-white" aria-current="page">Admin Portal</Link><Link href="/admin/customers?portal=customer" prefetch className={portalLinkClass}>Preview Customer Portal</Link><Link href="/admin/suppliers?portal=supplier" prefetch className={portalLinkClass}>Preview Supplier Portal</Link></nav>}<div className="hidden text-sm font-semibold xl:block">{title}</div><SignOutButton /></div></div></header><div className="container-shell grid gap-6 py-6 lg:grid-cols-[240px_1fr]"><aside className="card h-fit p-3">{nav.map((item, index) => {
